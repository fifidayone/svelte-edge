...
- `fly`: `x`, `y`, `opacity`
- `slide`: `axis: 'x' | 'y'` — animates `height` and needs block, flex, or grid layout; on `display: inline`, `inline-*`, `contents`, or `table` it does not animate and logs the `transition_slide_display` runtime warning in development.
- `scale`: `start`, `opacity`
...
### Manual-trigger pattern (Imperative)

Use a manual `.set(...)` call with `instant: prefersReducedMotion.current` when motion happens in an explicit event handler and should snap instantly for reduced-motion users:
...
<script lang="ts">
	...
	function snap() {
		x.set(100, { instant: prefersReducedMotion.current });
	}
</script>

<button onclick={snap}>Move</button>
```

### Reactive-bound pattern (Declarative)

When a spring or tween is bound to a reactive source with `Spring.of` or `Tween.of`, manual `.set(...)` is not enough because state changes automatically animate. Pass a reactive options object using **getters** as the second argument so the physics/timing adjust reactively to system motion settings:
...
<script lang="ts">
	...
	// Reactive spring options using getters
	const x = Spring.of(() => target, {
		get stiffness() {
			return prefersReducedMotion.current ? 1 : 0.15;
		},
		get damping() {
			return prefersReducedMotion.current ? 1 : 0.8;
		}
	});

	// Reactive tween options using getters
	const y = Tween.of(() => target, {
		get duration() {
			return prefersReducedMotion.current ? 0 : 400;
		}
	});
</script>

<div style={`transform: translate(${x.current}px, ${y.current}px)`}>box</div>
```

Use the manual-trigger pattern for event-driven updates. Use the reactive-bound pattern when the animation source is a reactive getter or prop.
...
Use `prefersReducedMotion` from `svelte/motion` (exposing `.current` boolean property) and short-circuit transitions when the user opts out:
...
- Respect `prefersReducedMotion.current` for accessibility — including inside `Spring.of` / `Tween.of` constructor options via reactive getters, not only in one-off `.set(...)` calls.
