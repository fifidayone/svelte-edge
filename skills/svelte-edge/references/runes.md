...
Do not destructure `$state` objects (e.g. `let { name } = $state({ name: 'Ada' })`); destructuring extracts the current value and loses reactivity. Read properties directly or use `$derived`.
- Write `obj.array ??= []; obj.array.push(...)` as two statements. Combined into one expression, `.push()` runs on the un-proxied right-hand side and the mutation is lost (`assignment_value_stale`).
- `raw === proxy` between an object and its `$state` proxy is always `false` (`state_proxy_equality_mismatch`) — compare proxy-to-proxy or raw-to-raw, or use `$state.snapshot()` on either side.
- Seeding local `$state` from a prop keeps only the initial value (`state_referenced_locally`) — use `$derived` if it should stay live, or `$state.snapshot(prop)` for a one-time value.
...
- Never mutate state inside `$derived(...)`, `$inspect(...)`, or a template expression — this is a **client error** (`state_unsafe_mutation`), not a warning. The error is thrown when the derived is first evaluated: a mutation inside a derived that is never read can pass silently, so the check is lazy — it does not fire at declaration time.
- A `$derived` created inside an `$effect` stops updating once that effect is destroyed (`derived_inert`) — declare it at the component top level or inside `$effect.root` instead.
...
Use `$effect.root(...)` only for advanced manual effect lifetimes. It returns a cleanup function that must be called to dispose the scope; failing to do so leaks memory in long-lived applications.
...
`$effect` tracks dependencies synchronously. State read after an `await` or inside `setTimeout` will not be tracked and will not trigger re-runs.
...
Do not mutate props you do not own — mutating non-bindable props triggers `ownership_invalid_mutation` warnings. Use callback props for one-way communication, or `$bindable()` only when the parent and child intentionally share writable state.
...
Use `$inspect.trace('label')` to trace what caused an effect or derived to re-run. It must be the first statement inside the function body.
...
In Svelte 5, runes can live outside components in `.svelte.ts` / `.svelte.js` files — use them for shared reactive helpers and app state that should live beyond a single component, but you **cannot export a directly reassignable `$state`**: the compiler rewrites references file-locally, so another module importing the binding would see a stale value. Three valid patterns:
...
```ts
// counter.svelte.ts — option 3: a factory when each consumer needs independent state
...
Options 1 and 2 share one module-wide state; the factory creates separate state per consumer.
...
### Reduced motion (`svelte/motion`)
- `prefersReducedMotion`: Reactive class (Svelte 5.7+) exposing `.current` (boolean) tracking `prefers-reduced-motion: reduce` OS settings.
...
- To read a legacy store in runes mode, use `fromStore(store)` from `'svelte/store'` for a reactive `.current` getter instead of manual `store.subscribe()`.
...
- For `Spring`, `Tween`, and transitions, read `references/motion.md`
