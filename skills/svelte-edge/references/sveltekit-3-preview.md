...
`page.state` is strictly typed on this line: declare each custom key on `App.PageState` in `app.d.ts` — reading an undeclared key fails `svelte-check` with `Property 'x' does not exist on type 'PageState'`.
...
- custom keys in the `details` object require extending `App.Error` in `app.d.ts` (e.g. `interface Error { code?: string }`) — without the extension the third parameter type collapses to `never` and `svelte-check` rejects the call.
